...
router.get('/status/down', async (req, res) => {
  try {
    const sql = `
      SELECT r.bts_name, r.bts_code, r.ip_address,
...
router.post('/', async (req, res) => {
  const { bts_name, ip_address, bts_code } = req.body;
  ...
  try {
    const sql = `
      INSERT INTO routers (bts_name, ip_address, bts_code) VALUES ($1, $2, $3)
      ON CONFLICT (ip_address) DO NOTHING RETURNING *
    `;
    const result = await query(sql, [bts_name.trim(), ip_address.trim(), bts_code ? bts_code.trim() : null]);
    if (result.rowCount === 0)
      return res.status(409).json({ success: false, error: `IP ${ip_address} already exists` });
    res.status(201).json({ success: true, data: result.rows[0] });
  } catch (err) {
    if (err.code === '23505') // unique violation (likely bts_code)
      return res.status(409).json({ success: false, error: `bts_code '${bts_code}' already exists` });
    serverError(res, err);
  }
});
...
// 7. PUT /api/routers/:ip — Update router (now accepts bts_code too)
router.put('/:ip', async (req, res) => {
  const ip = req.params.ip;
  const { bts_name, ip_address: newIp, bts_code } = req.body;
  if (!bts_name && !newIp && bts_code === undefined)
    return res.status(400).json({ success: false, error: 'Provide bts_name, ip_address, or bts_code to update' });
  try {
    ...
    if (bts_code !== undefined) { updates.push(`bts_code = $${idx++}`); vals.push(bts_code === null ? null : bts_code.trim()); }
    vals.push(ip);
